...
def convert(lexicon_dictionary, search_number) :
    for lexicon, number in lexicon_dictionary.items():
...
with graph.as_default():
    ...
    def step_op(time, memory_state, controller_state, inputs, outputs):
        ...
        with tf.variable_scope('controller'):
            #Xt = tf.concat(0, [step_input, tf.reshape(r, [-1])]) #original
            Xt = tf.concat([step_input, tf.reshape(r, [-1])], 0)
            nn_output, nn_state = network(Xt, controller_state)
            #std = lambda input_size: np.min(0.01, np.sqrt(2. / input_size)) #original
    ...
    with tf.Session(graph=graph) as session:
        ...
        for test_file in test_files:
            ...
            for story in test_data:
                ...
                if not writed :
                    for n in astory :
                       token = convert(lexicon_dictionary, n)                
                       print(token, end = ' ')
                       if token == "." or token == "-" :
                          print("\n", end='')
                
                    for n in desired_answers :
                        token = convert(lexicon_dictionary, n)                
                        answers.append(token)
                    print("Target :", answers)
                    
                    for n in given_answers :
                        token = convert(lexicon_dictionary, n)
                        predicts.append(token)
